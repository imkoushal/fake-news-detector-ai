"""
UI components — header, sidebar metrics, API status display.
"""
...
def render_sidebar_metrics(cfg: Dict):
    ...
    if cfg:
        ...
        training_date = cfg.get("training_date", "N/A")
        st.sidebar.caption(f"Training Date: {str(training_date)[:10]}")
        total_samples = cfg.get("total_training_samples", 0)
        st.sidebar.caption(f"Total Samples: {total_samples:,}" if isinstance(total_samples, int) else f"Total Samples: {total_samples}")


def render_api_status(gnews_ok: bool, gnews_msg: str, gemini_available: bool):
    ...
    if gemini_available:
        ...
        st.sidebar.caption("Set GEMINI_API_KEY in .env file")
